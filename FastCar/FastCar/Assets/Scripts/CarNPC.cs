...
public class CarNPC : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        varsayilanHiz = Random.Range(2.5f, 3.5f);
        gidilenSerit = Random.Range(1, 5);
    ...
    void FixedUpdate()
    {
        rb.velocity = new Vector3(rb.velocity.x, varsayilanHiz * 50 * Time.deltaTime, 0);
    }
    ...
    private void OnTriggerEnter2D(Collider2D sollama)
    {
        if (sollama.gameObject.tag=="Sollama")
        {
            ScorManager.skor += 25; 
        }
    }
